        ...
        try {
            for (int j = 1; j < dataset.size(); j++) {
                ...
                for (short i = 0; i < fields.length; i++) {
                    ...
                    if (!field.getType().getName().contains("oolean"))
                        getMethodName = "get" + fieldName.substring(0, 1).toUpperCase() + fieldName.substring(1);
                    else getMethodName = "is" + fieldName.substring(0, 1).toUpperCase() + fieldName.substring(1);
                    Method getMethod = t.getClass().getMethod(getMethodName);
//                    Object value = field.get(t);不能使用get获取private属性
